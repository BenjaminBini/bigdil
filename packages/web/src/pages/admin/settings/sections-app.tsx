...
export function UnassignedBehaviorSection({
  enabled,
  onChange,
}: UnassignedBehaviorSectionProps) {
  return (
    <SectionCard
    ...
    >
      <Toggle
        label="Use profile default cost rate for UNASSIGNED rows"
        checked={enabled}
        onChange={onChange}
      >
        When UNASSIGNED rows exist, use the profile's default cost rate for
        forecast calculations instead of leaving costs as zero.
      </Toggle>
    </SectionCard>
  );
}
