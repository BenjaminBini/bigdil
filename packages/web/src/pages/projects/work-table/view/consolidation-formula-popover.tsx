...
function fmtDateRange(start: string, end: string, dateFmt: Intl.DateTimeFormat): string {
...
interface FormulaContent {
  ...
  symbolic: string
  /** Same equation with values plugged in, e.g. `7j × 600 €/j` */
  withValues: string
  result: string
  description?: string
  factors?: Factor[]
...
function aggregateChildContributions(
  ...
  function collect(r: GridRow) {
    ...
    for (const child of rows) {
      if (child === r) continue
      if (r.kind === 'profile' && child.kind === 'employee' && child.phaseId === r.phaseId && child.taskId === r.taskId && child.profileId === r.profileId) collect(child)
      else if (r.kind === 'task' && child.kind === 'profile' && child.phaseId === r.phaseId && child.taskId === r.taskId) collect(child)
      else if (r.kind === 'phase' && child.kind === 'task' && child.phaseId === r.phaseId) collect(child)
      else if (r.kind === 'grand-total' && child.kind === 'phase') collect(child)
    }
  }
  collect(row)
...
function buildSourceSection(
  ...
  const contributions = row.kind === 'employee'
    ? buildCellContributions(row, periodsByKey, statusFilter, dateFmt)
    : aggregateChildContributions(row, rows, periodsByKey, statusFilter, dateFmt)
...
function quoteLinesForRow(row: GridRow, quotes: Quote[]) {
  ...
  const matches: Array<{ quoteId: string; title: string; days: number; sellRatePerDay: number; revenue: number; effectiveAt: string | null }> = []
...
function getChildren(row: GridRow, rows: GridRow[]): GridRow[] {
  switch (row.kind) {
    case 'profile':
      return rows.filter((r) => r.kind === 'employee' && r.phaseId === row.phaseId && r.taskId === row.taskId && r.profileId === row.profileId)
    case 'task':
      return rows.filter((r) => r.kind === 'profile' && r.phaseId === row.phaseId && r.taskId === row.taskId)
    case 'phase':
...
function buildFormulaContent(args: BuildArgs): FormulaContent | null {
  const { col, row, fd, rows, frozenData, prevRaf, prevSnapshotMonthCode, periodsByKey, quotes, t, dateFmt } = args
  const tp = (k: string, opts?: Record<string, unknown>) => t(`workTable.formulaPopover.${k}`, opts ?? {}) as string
  const children = getChildren(row, rows)
  const childLevelKey =
    row.kind === 'profile' ? 'employee' : row.kind === 'task' ? 'profile' : row.kind === 'phase' ? 'task' : row.kind === 'grand-total' ? 'phase' : null
  const childLevelLabel = childLevelKey ? tp(`levels.${childLevelKey}`) : null
  const detailHeading = childLevelLabel ? tp('detailBy', { level: childLevelLabel }) : null
  const childrenSum = tp('childrenSum', { count: children.length })

  const isFrozenOrConsolidation = (s: PeriodInfo['status']) => s === 'FROZEN' || s === 'CONSOLIDATION'
  const isOpenOrFuture = (s: PeriodInfo['status']) => s === 'OPEN' || s === 'FUTURE'
  const isConsolidation = (s: PeriodInfo['status']) => s === 'CONSOLIDATION'

  const quoteHint = (l: { effectiveAt: string | null; quoteId: string }): string =>
    l.effectiveAt
  ...
  switch (col.key) {
    case 'tc_spent':
      return {
        ...
        withValues: `${fmtDays(row.validatedDaysSpent)} + ${fmtDays(row.daysInConsolidation)}`,
        result: fmtDays(fd.tcDaysSpent),
        breakdown: detailHeading
          ? {
              heading: detailHeading,
              lines: children.map((c) => ({ label: c.label, expr: fmtDays(frozenData.get(c.id)?.tcDaysSpent ?? 0) })),
    ...
    case 'tc_rem':
      return {
        ...
        withValues: fmtDays(row.totalRemaining),
        result: fmtDays(fd.tcDaysRemaining),
        breakdown: detailHeading
          ? {
              heading: detailHeading,
              lines: children.map((c) => ({ label: c.label, expr: fmtDays(frozenData.get(c.id)?.tcDaysRemaining ?? 0) })),
              total: fmtDays(fd.tcDaysRemaining),
            }
          : undefined,
        cellSources: buildSourceSection(tp('sources.planning'), row, rows, periodsByKey, isOpenOrFuture, dateFmt),
      }
          ...
          factors: [
            {
              ...
              hint: row.employeeId ? tp('hints.costRateEmployee') : tp('hints.costRateProfile'),
            },
          ],
        }
      }
        ...
        breakdown: detailHeading
          ? {
              ...
              lines: children.map((c) => {
                ...
                return {
                  label: c.label,
                  expr: days > 0 ? `${fmtDays(days)} × ${fmtMoney(rate)} = ${fmtMoney(amount)}` : fmtMoney(amount),
                }
              }),
    ...
    case 'tr_sold': {
      ...
      return {
        ...
        breakdown: detailHeading
          ? {
              heading: detailHeading,
              lines: children.map((c) => ({ label: c.label, expr: fmtDays(frozenData.get(c.id)?.trDaysSold ?? 0) })),
              total: fmtDays(fd.trDaysSold),
            }
          : undefined,
      }
    ...
    case 'tr_amount': {
            ...
            ql.length > 0
              ? {
                  ...
                  lines: ql.map((l) => ({
                    ...
                    hint: l.effectiveAt ? tp('hints.effective', { date: fmtDate(l.effectiveAt, dateFmt) }) : undefined,
                  })),
      ...
      return {
        ...
        breakdown: detailHeading
          ? {
              heading: detailHeading,
              lines: children.map((c) => ({ label: c.label, expr: fmtMoney(frozenData.get(c.id)?.trAmount ?? 0) })),
              total: fmtMoney(fd.trAmount),
            }
          : undefined,
      }
    ...
    case 'tr_margin':
      return {
        ...
        factors:
          fd.trMarginPct != null
            ? [{ label: tp('factors.marginPct'), value: `${fd.trMarginPct.toFixed(1)}%`, hint: tp('hints.marginPctFormula') }]
            : undefined,
      }
    ...
    case 'pc_spent':
      return {
        ...
        breakdown: detailHeading
          ? {
              heading: detailHeading,
              lines: children.map((c) => ({ label: c.label, expr: fmtDays(frozenData.get(c.id)?.pcDaysSpent ?? 0) })),
    ...
    case 'pr_produced': {
      if (row.kind === 'profile') {
        ...
        return {
          ...
          factors: [
            { label: tp('factors.prevRaf'), value: fmtDays(baseline), hint: baselineHint },
            { label: tp('factors.currentRaf'), value: fmtDays(row.totalRemaining), hint: tp('hints.openFutureCells') },
          ],
        }
      }
      return {
        title: tp('titles.pr_produced'),
        symbolic: row.kind === 'employee' ? tp('symbolic.pr_produced_employee') : tp('symbolic.pr_produced_aggregate'),
        withValues: fmtDays(fd.prDaysProduced),
        result: fmtDays(fd.prDaysProduced),
        breakdown: detailHeading
          ? {
              heading: detailHeading,
              lines: children.map((c) => ({ label: c.label, expr: fmtDays(frozenData.get(c.id)?.prDaysProduced ?? 0) })),
    ...
    case 'pr_amount': {
      if (row.kind === 'profile') {
        return {
          ...
          factors: [{ label: tp('factors.sellRate'), value: fmtMoney(fd.trDailyRate), hint: tp('hints.sellRateFromQuote') }],
        }
      }
      return {
        title: tp('titles.pr_amount'),
    ...
    case 'pr_margin':
      return {
        ...
        factors:
          fd.prMarginPct != null
            ? [{ label: tp('factors.marginPct'), value: `${fd.prMarginPct.toFixed(1)}%`, hint: tp('hints.marginPctFormula') }]
            : undefined,
      }
...
function SourceTag({ source, t }: { source: CellSource; t: TFunction }) {
  ...
  return (
    <span className={`rounded px-1 py-0.5 text-[9px] font-semibold uppercase tracking-wide ${cls}`}>{label}</span>
  )
}
...
export function ConsolidationFormulaPopover({
  ...
  const content = open
    ? (() => {
        ...
        return buildFormulaContent({ col, row, fd, rows, frozenData, prevRaf, prevSnapshotMonthCode, periodsByKey, quotes, t, dateFmt })
      })()
    : null
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-[420px] max-h-[520px] overflow-y-auto p-0" align="end" side="top">
        {content ? (
          <div className="flex flex-col">
            ...
            <div className="border-b px-3 py-2">
              <div className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
                ...
                {' · '}
                {row.kind === 'quote' ? row.kind : t(`workTable.formulaPopover.levels.${row.kind}`)}
              </div>
            ...
            <div className="border-b bg-muted/30 px-3 py-2">
              <div className="font-mono text-[12px] leading-relaxed text-foreground">
                <div className="text-muted-foreground">{content.symbolic}</div>
                <div>
              ...
              {content.description && (
                <div className="mt-1.5 text-[11px] leading-snug text-muted-foreground">{content.description}</div>
              )}
            </div>
              ...
              <div className="flex flex-col border-b px-3 py-2">
                {content.factors.map((f) => (
                  <div key={f.label} className="flex flex-col py-0.5">
                    ...
                    {f.hint && <span className="pl-2 text-[10px] leading-tight text-muted-foreground/70">{f.hint}</span>}
                  </div>
                ))}
              </div>
            )}
            ...
            {content.breakdown && content.breakdown.lines.length > 0 && (
              <div className="flex flex-col border-b px-3 py-2">
                <div className="mb-1 text-[10px] uppercase tracking-wide text-muted-foreground">{content.breakdown.heading}</div>
                {content.breakdown.lines.map((line, idx) => (
                  <div key={`${line.label}-${idx}`} className="flex flex-col py-0.5">
                    <div className="flex items-baseline justify-between gap-2 text-xs">
                      <span className="truncate text-muted-foreground" title={line.label}>
                        {line.label}
                      </span>
                      <span className="shrink-0 font-mono tabular-nums text-foreground/90">{line.expr}</span>
                    </div>
                    {line.hint && <span className="pl-2 text-[10px] leading-tight text-muted-foreground/60">{line.hint}</span>}
                  </div>
                ))}
                {content.breakdown.total && (
                  <div className="mt-1 flex items-center justify-between border-t pt-1 text-[11px] font-semibold text-foreground">
                    ...
                    <span className="font-mono tabular-nums">{content.breakdown.total}</span>
                  </div>
                )}
              </div>
            )}
            ...
            {content.cellSources && content.cellSources.rows.length > 0 && (
              <div className="flex flex-col border-b px-3 py-2">
                ...
                {content.cellSources.rows.map((src) => (
                  <div key={src.periodKey} className="flex items-center justify-between gap-2 py-0.5 text-xs">
                    <div className="flex min-w-0 flex-col">
                      <span className="truncate text-foreground/80">{src.label}</span>
                      <span className="text-[10px] text-muted-foreground/70">{src.dateRange}</span>
                    </div>
                    <div className="flex shrink-0 items-center gap-1.5">
                      <SourceTag source={src.source} t={t} />
                      <span className="font-mono tabular-nums text-foreground/90">{fmtDays(src.days)}</span>
                    </div>
                  </div>
                ))}
                <div className="mt-1 flex items-center justify-between border-t pt-1 text-[11px] font-semibold text-foreground">
                  <span>{t('workTable.formulaPopover.total')}</span>
                  <span className="font-mono tabular-nums">{content.cellSources.total}</span>
                </div>
              </div>
            )}
          </div>
