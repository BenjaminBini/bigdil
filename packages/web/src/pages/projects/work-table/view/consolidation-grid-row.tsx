...
import { FROZEN_COLS, formatFrozenValue, getFrozenMarginPct, getFrozenValue, isFrozenColApplicable } from '@/lib/work-table/frozen'
...
export function ConsolidationGridRow({
  ...
  return (
    <tr className={cn('group', rowBg, row.kind === 'employee' && 'opacity-60', row.kind === 'grand-total' && 'border-t-2 border-row-divider')}>
      ...
      {FROZEN_COLS.map((col) => {
        const applicable = isFrozenColApplicable(row.kind, col.key)
        const value = applicable && detail ? getFrozenValue(detail, col.key) : null
        const isMarginCol = col.format === 'margin'
        const isNoData = value === null
        const isZero = value === 0
        const pct = applicable && detail && isMarginCol ? getFrozenMarginPct(detail, col.key) : undefined
        const cellInner = (
          <button
            type="button"
            className={cn(
          ...
          >
            ...
            {isMarginCol && pct != null && !isNoData && <MarginPctSuffix>{pct.toFixed(1)}%</MarginPctSuffix>}
          </button>
        )
        ...
        return (
          <td
            ...
            className={cn(
              ...
              rowBg,
              (isNoData || isZero) && 'text-muted-foreground/40',
              !isNoData && !isZero && !isMarginCol && 'text-foreground/80',
              isMarginCol && !isNoData && value! > 0 && 'text-emerald-600 dark:text-emerald-400',
              isMarginCol && !isNoData && value! < 0 && 'text-red-600 dark:text-red-400',
